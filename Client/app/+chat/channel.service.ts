...
import { MessageSubject } from './models/message-subject';
...
export class ChannelService {
   ...
   //
   private subjects = new Array<MessageSubject>();
   ...
   constructor(
      ...
      channelConfig.url = "http://localhost:56472/signalr";
      ...
      this.hubProxy.on("onEvent", (channel: string, ev: MessageEvent) => {
         ...
         //
         let channelSub = this.subjects.find((x: MessageSubject) => {
    ...
    * channel 
    * */
   sub(channel: string): Observable<MessageEvent> {
      ...
      //
      channelSub = new MessageSubject();
      channelSub.channel = channel;
      channelSub.subject = new Subject<MessageEvent>();
   ...
   unsubscribe(channel: string): void {
      ...
      this.hubProxy.invoke("Unsubscribe", channel)
         ...
         .fail((error: any) => {
            console.error("Error unsubscribing from chat room");
         });
   }
